    ...
    <ul class="reminders">
      <li v-for="reminder in availableReminders" :key="reminder.role + ' ' + reminder.name" class="reminder"
        ...
        <picture>
          ...
          <source :srcset="rolePath(reminder, 'svg')" type="image/svg+xml" />
          <img v-if="!reminder.image" :src="rolePath(reminder, 'png')" :alt="reminder.name" />
        </picture>
...
const availableReminders = computed((): Reminder[] => {
  ...
  reminders.push({
    id: "good",
    role: "good",
    image: new URL('../../assets/icons/good.png', import.meta.url).href,
    name: t('modal.reminder.good'),
  });
  reminders.push({
    id: "evil",
    role: "evil",
    image: new URL('../../assets/icons/evil.png', import.meta.url).href,
...
function rolePath(reminder: Reminder, format: string) {
  return new URL(
    `../../assets/icons/${reminder.role}.${format}`,
    import.meta.url,
  ).href;
}
